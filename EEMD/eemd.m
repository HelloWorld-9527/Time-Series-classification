...
%       of Y;%%噪音比率？【添加的噪声的标准差与Y的比值__by GerHard】
%       NE: Ensemble number for the EEMD%%集合数？
% OUTPUT:
...
function allmode=eemd(Y,Nstd,NE)%输入数据，噪音比率，集合数
xsize=length(Y);
dd=1:1:xsize;
Ystd=std(Y);%%std为Y的高斯分布中抽样出来的数据
Y=Y/Ystd;
